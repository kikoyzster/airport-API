...
namespace airport.Controllers
{
    public class ArrivalsController : Controller
    {
        ...
        public ArrivalsController(IApiService apiService, IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IActionResult> Index(DateTime date, string time)
        {
           ...
           // HttpContext.Session.SetString("SelectedArrivalDate", airportCode ?? "Default");
            ...
            var _url = $"https://api.swedavia.se/flightinfo/v2/{iata}/arrivals/{date:yyyy-MM-dd}";


            var arrivals = await client.GetFromJsonAsync<Arrivals>(_url);
            arrivals.selectedDateStr = date.ToShortDateString();
            arrivals.selectedTimeStr = time;
            return View(arrivals);
        }
        public async Task<IActionResult> FlightInfo(ArrivalFlight arrivalFlight)
        {
            SubscriptionKeyFlightInfo subscriptionKeyFlightInfo = new SubscriptionKeyFlightInfo();
            var flightInfoKey = SubscriptionKeyFlightInfo.getKey();
            ...
            DateTime date = DateTime.Parse(arrivalFlight.scheduledUtc);
            var _url = $"https://api.swedavia.se/flightinfo/v2/{iata}/arrivals/{date:yyyy-MM-dd}";
            

            var arrivals = await client.GetFromJsonAsync<Arrivals>(_url);
            ArrivalFlight selectedFlight = arrivals.flights.Where(flight => flight.flightId == arrivalFlight.flightId).FirstOrDefault();
            selectedFlight.arrivalTime.scheduledUtcShort = DateTime.Parse(arrivalFlight.scheduledUtc).ToShortTimeString();
